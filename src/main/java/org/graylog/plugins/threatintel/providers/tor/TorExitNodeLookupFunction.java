...
public class TorExitNodeLookupFunction extends AbstractFunction<TorExitNodeLookupResult> {
    ...
    @Override
    public TorExitNodeLookupResult evaluate(FunctionArgs args, EvaluationContext context) {
    ...
    @Override
    public FunctionDescriptor<TorExitNodeLookupResult> descriptor() {
        return FunctionDescriptor.<TorExitNodeLookupResult>builder()
                .name(NAME)
                .description("Match an IP address against known Tor exit nodes to identify connections from the Tor network.")
                .params(valueParam)
                .returnType(TorExitNodeLookupResult.class)
                .build();
    }
